...
from dataloader import BDDADataset_seg
...
class Test():
	...
	def load_saved(self, config_dir):
		...
		elif self.dataset_name == 'BDD-A' or self.dataset_name == 'BDDADataset_seg':
	...
	def test_saved(self, config_dir, evaluate=True, save_images=False):
		...
		if self.dataset_name == 'DReyeVE':
			...
			test_dataset = BDDADataset_seg(self.model_params['clip_size'],
										   mode='test',
										   weight_type=self.train_params['weight_type'],
										   img_size=self.model_params['img_size'],
										   task_attributes=self.task_attributes,
	...
	def eval_batch(self, frame_idx, pred_sal, gt_sal, eval_dict):
		...
		cc_loss = cc(pred_sal, gt_sal).detach().cpu().numpy()
		sim_loss   = sim(pred_sal, gt_sal).detach().cpu().numpy()
		nss_loss   = nss(pred_sal, gt_sal).detach().cpu().numpy()
		kldiv_loss = kldiv(pred_sal, gt_sal).detach().cpu().numpy()
